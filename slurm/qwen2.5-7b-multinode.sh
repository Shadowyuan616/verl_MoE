...
#SBATCH --job-name=FlowRL_qwen2.5_7B_math_train
#SBATCH -N 2
#SBATCH --ntasks-per-node=1           # 改为1
#SBATCH --cpus-per-task=64            # 添加CPU核心数
#SBATCH --exclude=g0013,g0014,g0016,g0017,g0018,g0024             
#SBATCH --gres=gpu:8                  
#SBATCH --mem=960G
...
# # 2.0 [新增] 强力清理残留进程
# # 必须先清理环境，防止端口冲突 (ConnectionError)
# echo "Cleaning up old Ray processes..."
# # 忽略报错 (|| true)，因为如果没有残留进程 killall 会报错
# srun --overlap --nodes=${SLURM_NNODES} --ntasks-per-node=1 killall -9 ray || true
# # 等待几秒确保端口释放
# sleep 5
...
srun --overlap --nodes=1 --ntasks=1 -w "$head_node" \
    bash -c "unset ROCR_VISIBLE_DEVICES; exec ray start --head --node-ip-address=$head_node_ip --port=$RAY_PORT --num-gpus=8 --dashboard-host=0.0.0.0 --block" &
...
for ((i=1; i<${#nodes_array[@]}; i++)); do
    ...
    srun --overlap --nodes=1 --ntasks=1 -w "$worker_node" \
        bash -c "unset ROCR_VISIBLE_DEVICES; exec ray start --address=$head_node_ip:$RAY_PORT --num-gpus=8 --block" &
done
...
script_path="./recipe/flowrl/run_flowrl_qwen2.5_7b.sh"
