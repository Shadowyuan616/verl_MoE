...
# CPU offload fraction: 1.0 = all on CPU, 0.5 = half on GPU half on CPU
# Reduced from 1.0 to 0.5 to avoid CPU RAM OOM on 1TB nodes
# This saves ~90 GB CPU per node by keeping 50% optimizer states on GPU
...
# optimizer_offload_fraction moved to line ~63 (set to 0.5 for CPU RAM savings)
...
exp_name="DAPO-FlowRL-Qwen3-30B-MATH-megatron-Node${NNODES}_bs${train_prompt_bsz}_PP${COMMON_PP}TP${COMMON_TP}EP${COMMON_EP}_INFER_TP${INFER_TP}_minbs${train_prompt_mini_bsz}"
...
TEST_FILE=${TEST_FILE:-"${WORKING_DIR}/dataset/aime-2024.parquet"}
    ...
    # actor_rollout_ref.model.override_config.output_router_logits=True \
    # actor_rollout_ref.actor.collect_moe_metrics=True \
    # actor_rollout_ref.actor.moe_top_k=8 \
    # actor_rollout_ref.actor.grad_clip=1.0 \  can't be used with megatron
